...
static wiced_result_t mqtt_network_recv_thread( void *socket );
static wiced_result_t mqtt_disconnect_callback( void *socket );
...
extern int g_mqtt_thread_is_running;
///////////////////////////////////////////
/******************************************************
...
wiced_result_t mqtt_core_init( mqtt_connection_t *conn )
{
    ...
    if ( ( result = wiced_rtos_create_thread( &mqtt_socket->net_thread, 10, "Mqttmain", mqtt_thread_main, 3100, mqtt_socket ) ) != WICED_SUCCESS )
    {
    ...
    if ( security != NULL )
    {
        ...
        if ( ( security->cert != NULL ) && ( security->key != NULL ) )
        {
            ...
            */
			...
			void *pTLSContext = malloc_named("TLS-Context", sizeof(wiced_tls_advanced_context_t));

			memset(pTLSContext, 0, sizeof(wiced_tls_advanced_context_t));

			((wiced_tls_advanced_context_t*)pTLSContext)->context_type = WICED_TLS_ADVANCED_CONTEXT;

			platform_dct_security_t const* dct_security = wiced_dct_get_security_section( );

			if ((result = wiced_tls_init_advanced_context(pTLSContext, dct_security->certificate, dct_security->private_key ))!=WICED_SUCCESS )
			{
				//W_DBG("SocketProcess : wiced_tls_init_context failed (%d)", result);
				goto ERROR_TLS_INIT;
			}

			if ( (result = wiced_tcp_enable_tls(&socket->socket, pTLSContext))!=WICED_SUCCESS )
				;
...
//            wiced_tls_init_context( &socket->tls_context, NULL, NULL );
            wiced_tcp_enable_tls(&socket->socket, NULL);
...
//    result = wiced_tcp_register_callbacks( &socket->socket, NULL, mqtt_network_recv_thread, mqtt_disconnect_callback, socket );
    result = wiced_tcp_register_callbacks( &socket->socket, NULL, mqtt_network_recv_thread, mqtt_disconnect_callback);
...
//    mqtt_socket_t *mqtt_socket = (mqtt_socket_t *) args;
	mqtt_socket_t *mqtt_socket = (mqtt_socket_t *) &(g_p_mqtt_connection->socket);

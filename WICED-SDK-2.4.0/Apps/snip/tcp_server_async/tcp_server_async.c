 ...
 *      shown in wifi_config_dct.h : WICED TCP Server Async App / 12345678
...
wiced_result_t client_connected_callback ( void* socket )
{
    ...
    if( result == WICED_SUCCESS )
    {
        ...
        wiced_tcp_server_peer( tcp_socket, &ipaddr, &port );
...
wiced_result_t client_disconnected_callback ( void* socket )
{
    wiced_tcp_socket_t* tcp_socket = (wiced_tcp_socket_t*)socket;
    ...
    if ( wiced_tcp_listen( tcp_socket, TCP_SERVER_LISTEN_PORT ) != WICED_SUCCESS )
    {
...
wiced_result_t received_data_callback( void* socket )
{
    wiced_result_t      result;
    ...
    result = wiced_tcp_receive( tcp_socket, &rx_packet, WICED_WAIT_FOREVER );
    if ( result != WICED_SUCCESS )
    {
        return result;
    }
    ...
    /* Send echo back */
    if ( wiced_packet_create_tcp( tcp_socket, TCP_PACKET_MAX_DATA_LENGTH, &tx_packet, (uint8_t**)&tx_data, &available_data_length ) != WICED_SUCCESS )
    {
    ...
    /* Send the TCP packet */
    if ( wiced_tcp_send_packet( tcp_socket, tx_packet ) != WICED_SUCCESS )
    {
...
void application_start(void)
{
    ...
    result = wiced_tcp_register_callbacks( &tcp_server_socket, client_connected_callback, received_data_callback, client_disconnected_callback );

...
void start_dhcp_server( struct sockaddr_in * local_addr );
...
void start_dhcp_server( struct sockaddr_in * local_addr )
{
    xTaskCreate( dhcp_thread, (signed char*)"DHCP thread", DHCP_STACK_SIZE/sizeof( portSTACK_TYPE ), local_addr, DEFAULT_THREAD_PRIO, &dhcp_thread_handle);
}
...
static void dhcp_thread( void * thread_input )
{
    struct sockaddr_in*   my_addr = (struct sockaddr_in*) thread_input;

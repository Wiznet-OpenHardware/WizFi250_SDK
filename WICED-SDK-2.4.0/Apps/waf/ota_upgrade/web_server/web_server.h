...
typedef void (*upload_processor_t)( void* data, int len);
...
typedef struct
{
    ...
    const upload_processor_t upload_processor;
} url_list_elem_t;
...
void run_webserver( void * bind_address_in, const url_list_elem_t * server_url_list_in );



void start_web_server_thread( void * bind_address_in, const url_list_elem_t * server_url_list_in );
wiced_bool_t web_server_is_running( void );
void quit_web_server( void );


#ifndef WEB_SERVER_NO_PRINT
#define WEB_SERVER_ERROR_PRINT( x )  WPRINT_WEBSERVER_ERROR( ( x ) )
#define WEB_SERVER_PRINT( x )        WPRINT_WEBSERVER_INFO( x )
#define WEB_SERVER_STACK_SIZE        (4 * 1024)
#else
#define WEB_SERVER_ERROR_PRINT( x )  wiced_assert( x, 0!=0 )

...
wl_ioctl(void *wl, int cmd, void *buf, int len, bool set)
{
	...
	void* ioctl_data = wiced_get_ioctl_buffer( &internalPacket, len );
	memcpy( ioctl_data, buf, len );
	ret = wiced_send_ioctl( (set==TRUE)?SDPCM_SET:SDPCM_GET, cmd, internalPacket, &response_buffer, SDPCM_STA_INTERFACE );
	if (ret == WICED_SUCCESS)
	{
		if (set!=TRUE)
		{
			memcpy( buf, host_buffer_get_current_piece_data_pointer( response_buffer ), host_buffer_get_current_piece_size( response_buffer )  );
		}
		host_buffer_release(response_buffer, WICED_NETWORK_RX);
	}

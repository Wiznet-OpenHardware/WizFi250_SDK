...
uint8_t g_wxProfile_mext1_1 = 0; // sekim XXXXXX
    ...
    spi_mapping[spi->port].peripheral_clock_func( spi_mapping[spi->port].peripheral_clock_reg,  ENABLE );
{
	...
	if ( g_wxProfile_mext1_1!=11 ) // sekim XXXXXX
		SPI_I2S_ITConfig(SPI1, SPI_I2S_IT_RXNE, ENABLE);
...
void spi1_rx_dma_irq(void)
{
	...
	sprintf(szXXXX3, "XXX 200 : (%d, %d)\r\n", (int)spi_int, (int)spi_rx_buffer_cnt);
...
void spi_dma_thread_hndlr( uint32_t arg )
{
	...
	while(1)
	{
		...
		//sprintf(szXXXX3, "(%d, %d)", (int)free_len, (int)spi_rx_suspend);
		//strcat(szXXXX2, szXXXX3);
		...
		{
			static uint32_t xxxx_len = 0;
			xxxx_len += (uint32_t)l_ndtr;
			sprintf(szXXXX3, "XXX 210 : (%lu, %d)\r\n", xxxx_len, (int)l_ndtr);
			strcat(szXXXX2, szXXXX3);

...
static const configuration_entry_t app_config[] =
{
    ...
    {"Zipcode",    DCT_OFFSET(config_mode_app_dct_t, zip),      1, CONFIG_UINT32_DATA },
    {"Phone",      DCT_OFFSET(config_mode_app_dct_t, phone),   20, CONFIG_STRING_DATA },
    {0,0,0,0}
};

wiced_bool_t is_configured=WICED_FALSE;

#define IS_CONFIGURED_INDEX (0)
...
void application_start( )
{
    ...
    {
        WPRINT_APP_INFO(("Can't join with current Wi-Fi DCT. Starting config mode again.\r\n"));

        /* Write into Wifi DCT that device needs to to be configured again.  */
        bootloader_api->write_wifi_config_dct( IS_CONFIGURED_INDEX, &is_configured, sizeof(is_configured));

        /* Try to reconfigure device again */
        wiced_configure_device( app_config );
    }
    ...
    WPRINT_APP_INFO( ( "First Name = %s\r\n",               ( ( config_mode_app_dct_t* )wiced_dct_get_app_section( ) )->fname   ) );
    WPRINT_APP_INFO( ( "Last Name  = %s\r\n",               ( ( config_mode_app_dct_t* )wiced_dct_get_app_section( ) )->lname   ) );
    WPRINT_APP_INFO( ( "Address 1  = %s\r\n",               ( ( config_mode_app_dct_t* )wiced_dct_get_app_section( ) )->addr1   ) );
    WPRINT_APP_INFO( ( "Address 2  = %s\r\n",               ( ( config_mode_app_dct_t* )wiced_dct_get_app_section( ) )->addr2   ) );
    WPRINT_APP_INFO( ( "Suburb     = %s\r\n",               ( ( config_mode_app_dct_t* )wiced_dct_get_app_section( ) )->suburb  ) );
    WPRINT_APP_INFO( ( "Country    = %s\r\n",               ( ( config_mode_app_dct_t* )wiced_dct_get_app_section( ) )->country ) );
    WPRINT_APP_INFO( ( "Zipcode    = %d\r\n", (unsigned int)( ( config_mode_app_dct_t* )wiced_dct_get_app_section( ) )->zip     ) );
    WPRINT_APP_INFO( ( "Phone      = %s\r\n",               ( ( config_mode_app_dct_t* )wiced_dct_get_app_section( ) )->phone   ) );

...
#if 0 //MikeJ 130507 ID1063 - USART HW Flow Control problem
...
#else
    uint32_t amount_to_copy = MIN(data_length, (tail_to_end + ring_buffer->head - 1) % ring_buffer->size);
#endif
...
uint32_t ring_buffer_free_space( wiced_ring_buffer_t* ring_buffer )
{
    ...
    uint32_t tail_to_end = ring_buffer->size - ring_buffer->tail;
#else
    uint32_t tail_to_end = ring_buffer->size - ring_buffer->tail - 1;
#endif

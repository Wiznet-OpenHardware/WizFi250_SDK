...
extern void start_dns_server( struct sockaddr_in* local_addr );
extern void quit_dns_server( void );
extern void start_dhcp_server( struct sockaddr_in* local_addr );
...
static void run_sta_web_server( void )
{
...
#endif /* ifdef APPLIANCE_ENABLE_WPS */
    {
        if ( appliance_config.config_type == CONFIG_SCANJOIN )
        {
            ...
            while ( WICED_SUCCESS != wiced_wifi_join_specific( &appliance_config.vals.scanjoin.scanresult, (uint8_t*) appliance_config.vals.scanjoin.passphrase, appliance_config.vals.scanjoin.passphrase_len, NULL ) )
            {
    ...
    if ( NULL == netif_add( &wiced_if, &ipaddr, &netmask, &gw, (void*) WICED_STA_INTERFACE, ethernetif_init, ethernet_input ) )
    {
    ...
    /* Construct the local IP address for the DHCP server */
    struct sockaddr_in my_address;
    my_address.sin_family = AF_INET;
    *( (uint32_t*) &my_address.sin_addr ) = wiced_if.ip_addr.addr;

    run_webserver( &my_address, config_STA_url_list );
    ...
    wiced_wifi_leave( );
    if ( WICED_SUCCESS != wiced_management_wifi_off( ) )
    {
...
static void run_ap_webserver( void )
{
    ...
    if ( NULL == netif_add( &wiced_if, &ap_ipaddr, &ap_netmask, &ap_ipaddr, (void*) WICED_AP_INTERFACE, ethernetif_init, ethernet_input ) )
    {
        WPRINT_APP_ERROR( ( "Failed to start network interface\r\n" ) );
        return;
    }
    ...
    struct sockaddr_in my_address;
    my_address.sin_family = AF_INET;
    *( (uint32_t*) &my_address.sin_addr ) = ap_ipaddr.addr;

    /* Create DNS and DHCP servers */
    start_dns_server( &my_address );
    start_dhcp_server( &my_address );

    run_webserver( &my_address, config_AP_url_list );
    ...
    if ( WICED_SUCCESS != wiced_wifi_stop_ap( ) )
    {

...
int main( void )
{
    ...
    while ( WICED_SUCCESS != ( result = wiced_management_init( COUNTRY, &pkt_buff_init ) ) )
    {
    ...
    while ( wiced_wifi_join( AP_SSID, AP_SEC, (uint8_t*) AP_PASS, sizeof( AP_PASS ) - 1, NULL ) != WICED_SUCCESS )
    {
    ...
    while ( 1 )
    {
        ...
        for ( i = 0; i < 10; i++ )
        {
            ...
            wiced_poll_all( ); /* Poll for packets to receive (which will be dropped) - 802.11 device will run out of memory if packets are not read out of it */
        }
    }
}
...
static void resolve_dest_mac( uint32_t dest_ip_addr, wiced_mac_t * MAC_buffer )
{
    ...
    do
    {
        ...
        host_buffer_set_data_end( (wiced_buffer_t) ( pkt_buffer + sizeof(wiced_buffer_header_t) ), (uint8_t*) ( pkt_buffer + sizeof(arp_packet_t) ) );

        /* Queue the packet in the transmit queue */
        wiced_network_send_ethernet_data( (wiced_buffer_t) &pkt->ethernet_header, WICED_STA_INTERFACE );
...
static void send_canned_packet( char* pkt, uint16_t payload_len )
{
    ...
    /* Set the packet size */
    host_buffer_set_data_end( pkt, (uint8_t*) ( pkt + sizeof(ethernet_header_t) + sizeof(ipv4_header_t) + sizeof(udp_header_t) + payload_len ) );

    /* Send the packet */
    wiced_network_send_ethernet_data( (wiced_buffer_t) &udp_pkt->ethernet_header, WICED_STA_INTERFACE );
    wiced_send_one_packet( ); /* Send packet only - do not poll all, since that would cause receives which will overwrite packet buffer contents */
}
 ...
 */
void host_network_process_ethernet_data( /*@only@*/ wiced_buffer_t p, wiced_interface_t interface )
{
    ethernet_header_t * ether_header = (ethernet_header_t *) host_buffer_get_current_piece_data_pointer( p );

    if ( ( interface != WICED_STA_INTERFACE ) ||                                /* Check that packet came from STA interface */
    ...
    {
        host_buffer_release( p, WICED_NETWORK_RX );
        return;
    }

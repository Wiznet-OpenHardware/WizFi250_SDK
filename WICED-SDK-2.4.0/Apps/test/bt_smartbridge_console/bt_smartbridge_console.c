...
static wiced_bt_smart_scan_result_t* directed_advertisement_result_ptr = NULL;
...
static wiced_result_t directed_advertisement_handler( const wiced_bt_smart_scan_result_t* directed_advertisement_result )
{
     ...
     */
    directed_advertisement_result_ptr = (wiced_bt_smart_scan_result_t*)directed_advertisement_result;
...
int start_iap( int argc, char *argv[] )
{
#ifdef USE_APPLE_MFI_IAP
...
int auto_test_smartbridge( int argc, char *argv[] )
{
    ...
    for ( b = 0; b < iteration; b++ )
    {
        ...
        for ( a = 0; a < count && a < MAX_BT_SMART_CONNECTIONS; a++ )
        {
            ...
            if ( b % 10 == 0 )
            {
                wiced_bt_smartbridge_enable_pairing( &smartbridge_socket[a], &security_settings, pairing_handler );
            }
            else
            {
...
int connect( int argc, char *argv[] )
{
    ...
    if ( enable_encryption != 0 )
    {
        ...
        {
            wiced_bt_smartbridge_enable_pairing( &smartbridge_socket[socket_number], &security_settings, pairing_handler );
        }
    }
    else
    {
...
int scan_connect_direct_adv( int argc, char *argv[] )
{
    ...
    /* Register a directed advertisement callback */
    ...
    if ( directed_advertisement_result_ptr != NULL )
    {
        if ( enable_encryption != 0 )
        {
            wiced_bt_smart_bond_info_t bond_info;

            if ( find_bond_info( &directed_advertisement_result_ptr->remote_device.address, directed_advertisement_result_ptr->remote_device.address_type, &bond_info ) == WICED_SUCCESS )
            {
                wiced_bt_smartbridge_set_bond_info( &smartbridge_socket[socket_number], &security_settings, &bond_info );
            }
            else
        ...
        if ( wiced_bt_smartbridge_connect( &smartbridge_socket[socket_number], &directed_advertisement_result_ptr->remote_device, &connection_settings, disconnection_handler, notification_handler ) == WICED_SUCCESS )
        {
            WPRINT_APP_INFO( ( "Connecting socket %lu successful. Connection handle: %lu\r\n", socket_number, (uint32_t)smartbridge_socket[socket_number].connection_handle ) );
        }
        else
    ...
    wiced_bt_smartbridge_register_directed_advertising_callback( NULL );
    directed_advertisement_result_ptr = NULL;
...
int clear_bond_info_dct( int argc, char *argv[] )
{
    ...
    /* Write updated bond info to DCT */
    result = wiced_dct_write_app_section( (const void*)&dct_local_copy, sizeof( dct_local_copy ) );

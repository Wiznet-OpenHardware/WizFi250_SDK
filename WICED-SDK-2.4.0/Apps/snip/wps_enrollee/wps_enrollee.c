...
static const wiced_wps_device_detail_t enrollee_details =
{
    ...
    .sub_category    = 7,
    .config_methods  = WPS_CONFIG_LABEL | WPS_CONFIG_PUSH_BUTTON | WPS_CONFIG_VIRTUAL_PUSH_BUTTON | WPS_CONFIG_PHYSICAL_PUSH_BUTTON | WPS_CONFIG_VIRTUAL_DISPLAY_PIN
};
...
void application_start( )
{
    ...
    if ( wiced_wps_enrollee( WICED_WPS_PBC_MODE, &enrollee_details, "00000000", credential, CONFIG_AP_LIST_SIZE ) == WICED_SUCCESS )
    {
        uint32_t i;
        ...
        {
            memcpy( &wifi_config.stored_ap_list[ i ].details.SSID, &credential[ i ].ssid, sizeof(wiced_ssid_t) );

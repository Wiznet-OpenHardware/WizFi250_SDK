...
int ping( int argc, char *argv[] )
{
    ...
    while (( num > 0 ) || ( continuous == WICED_TRUE ) )
    {
        ...
        if ( ( wiced_wifi_is_ready_to_transceive(WICED_AP_INTERFACE) == WICED_SUCCESS ) && ( status != WICED_SUCCESS ) )
        {
...
//    status = _nx_packet_data_append(request_ptr, (VOID *) data_ptr, data_size, ip_ptr->nx_ip_default_packet_pool, 2000);
//
...
//    data_offset = 0;
    ...
    /* Allocate a packet to place the ICMP echo request message in.  */
    status =  nx_packet_allocate(&wiced_packet_pools[1], &request_ptr, NX_ICMP_PACKET, wait_option);
    if (status)
    {
...
#ifdef ENABLE_LONG_PING
    /* Now, copy the data into the NetX packet.  */
    status = nx_packet_data_append(request_ptr, (VOID *) data_ptr, data_size, &wiced_packet_pools[1], 2000);

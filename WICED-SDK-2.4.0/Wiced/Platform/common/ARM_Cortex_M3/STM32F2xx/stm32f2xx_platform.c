...
wiced_result_t wiced_spi_init( const wiced_spi_device_t* spi )
{
    ...
    if(g_custom_gpio == CST_OPEN_DRAIN )		// kaizen 20140430 ID1173 - Added Function of Custom GPIO TYPE
    {
        gpio_init_structure.GPIO_Mode  = GPIO_Mode_AF;
        gpio_init_structure.GPIO_OType = GPIO_OType_PP;
    ...
    if (uart_interfaces[uart].rx_buffer != NULL)
    {
        ...
        while (size != 0)
        {
            ...
            if ( platfrom_spi_stdio_x!=platfrom_spi_stdio )
            {
           		...
           		// sekim XXXXXX 20150609 can't fix platfrom_spi_stdio
           		//quick_change_sdtio_mode(platfrom_spi_stdio_x);
           		if ( platfrom_spi_stdio==0 ) 	quick_change_sdtio_mode(platfrom_spi_stdio_x);
            }
#endif

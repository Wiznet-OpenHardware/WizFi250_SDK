...
#ifndef BUILD_WIZFI250 //MikeJ 130528 ID1072 - Improve UART speed (AT+SSEND)
#define STDIO_BUFFER_SIZE   64
#else
#define STDIO_TX_BUFFER_SIZE   512
#define STDIO_RX_BUFFER_SIZE   2048
#endif
#endif
...
#ifdef BUILD_WIZFI250 // sekim 20131125 Add SPI Interface
#define SPI_RX_XON_POINT			( 128 )
#define SPI_RX_XOFF_POINT			( 32 )
...
// sekim XXXXXXX
#define DMA_BURST_SIZE	1024	// 2013.12.18 sskim
#define DMA_BUFFER_SIZE	5		// 2014.01.06 sskim
#endif
...
// sekim XXXXXXX
#ifdef BUILD_WIZFI250 	// 2014.01.07 sskim
typedef struct {
	uint8_t		buf[DMA_BUFFER_SIZE][DMA_BURST_SIZE];
	uint32_t	buf_len[DMA_BUFFER_SIZE];
	uint32_t	push;
	uint32_t	pop;
	uint32_t	len;
...
// 2013.12.18 sskim
void dma_polling_thread_hndlr( uint32_t arg );
void spi_dma_thread_hndlr( uint32_t arg );

// 2014.01.07 sskim
#ifdef BUILD_WIZFI250 	// 2014.01.07 sskim
void init_spi_dma_buffer(SPI_DMA_BUFFER *q);
void push_dma_data(SPI_DMA_BUFFER *q, uint32_t buf_len);
int pop_dma_data(SPI_DMA_BUFFER *q, uint8_t *buf, uint32_t *len);
#endif
////////////////////////////////////////////////////////
...
#ifdef BUILD_WIZFI250 //MikeJ 130418 ID1007 - USART Rx sync problem
static wiced_ring_buffer_t stdio_rx_buffer;
static wiced_ring_buffer_t stdio_tx_buffer;
static uint8_t             stdio_rx_data[STDIO_RX_BUFFER_SIZE];
static uint8_t             stdio_tx_data[STDIO_TX_BUFFER_SIZE];
#else
...
#define SPI_GPIO_CLEAR_COUNT	2
#define CTRL_REQ	2
#define CTRL_RDY	1
#define CTRL_NON	0

//static uint8_t spi_stdio = DISABLE;
static  int8_t spi_tx_sig_cnt = 0;
static uint8_t spi_tx_suspend  = CTRL_NON;
static uint8_t spi_rx_suspend  = CTRL_NON;
static uint8_t spi_rx_previous = CTRL_NON;
static wiced_bool_t spi_rx_stop = WICED_FALSE;

uint8_t platfrom_spi_stdio = 0; // g_wxProfile.spi_stdio, // 0:Auto, 1:RX(UART)TX(UART) 2:RX(SPI)TX(SPI)
uint8_t platfrom_spi_stdio_x = 0;

extern uint8_t g_spix_bulkmode;
...
#ifdef BUILD_WIZFI250	// 2013.12.20 sskim sekim XXXXXX
host_semaphore_type_t	dma_mutex;
#endif
...
#ifndef BUILD_WIZFI250 //MikeJ 130528 ID1072 - Improve UART Performance
...
#else
    ring_buffer_init  ( (wiced_ring_buffer_t*)&stdio_rx_buffer, (uint8_t*)stdio_rx_data, STDIO_RX_BUFFER_SIZE );
	ring_buffer_init  ( (wiced_ring_buffer_t*)&stdio_tx_buffer, (uint8_t*)stdio_tx_data, STDIO_TX_BUFFER_SIZE );
    platform_uart_init( STDIO_UART, &stdio_uart_config, (wiced_ring_buffer_t*)&stdio_rx_buffer );
#endif
#endif

#ifdef BUILD_WIZFI250		// 2013.12.20 sskim // sekim XXXXXX
	host_rtos_init_semaphore(&dma_mutex);
#endif
...
/////////////////////////////////////////////////////////////////
// sekim XXXXXXXXXX
#ifdef BUILD_WIZFI250 	// 2014.01.06 sskim
void init_spi_dma_buffer(SPI_DMA_BUFFER *q)
{
	q->push = 0;
	q->pop = 0;
	...
	memset(q->buf_len, 0, sizeof(q->buf_len));
}

void push_dma_data(SPI_DMA_BUFFER *q, uint32_t buf_len)
{
	if(q->len >= DMA_BUFFER_SIZE)
	{
		q->overflow++;
		q->pop++;
		q->pop = q->pop % DMA_BUFFER_SIZE;
	}
	else
		q->len++;

	q->buf_len[q->push] = buf_len;
	q->push++;
	q->push = q->push % DMA_BUFFER_SIZE;
}
...
int pop_dma_data(SPI_DMA_BUFFER *q, uint8_t *buf, uint32_t *len)
{
	...
	*len = q->buf_len[q->pop];
	memcpy(buf, q->buf[q->pop], q->buf_len[q->pop]);
...
SPI_DMA_BUFFER spi_rx_dma_buffer;
#endif		// 2014.01.06 sskim
/////////////////////////////////////////////////////////////////
...
wiced_result_t wiced_spi_init( const wiced_spi_device_t* spi )
{
...
#ifndef BUILD_WIZFI250 // sekim 20131125 Add SPI Interface
    /* Init SPI GPIOs */
    ...
    gpio_init_structure.GPIO_Pin   = ((uint32_t) (1 << spi_mapping[spi->port].pin_clock->number)) |
                                     ((uint32_t) (1 << spi_mapping[spi->port].pin_miso->number )) |
                                     ((uint32_t) (1 << spi_mapping[spi->port].pin_mosi->number )) |
                                     ((uint32_t) (1 << gpio_mapping[spi->chip_select].number  ));
    GPIO_Init( spi_mapping[spi->port].pin_clock->bank, &gpio_init_structure );

    GPIO_PinAFConfig( spi_mapping[spi->port].pin_clock->bank, spi_mapping[spi->port].pin_clock->number,  spi_mapping[spi->port].gpio_af );
    GPIO_PinAFConfig( spi_mapping[spi->port].pin_miso->bank,  spi_mapping[spi->port].pin_miso->number,   spi_mapping[spi->port].gpio_af );
    GPIO_PinAFConfig( spi_mapping[spi->port].pin_mosi->bank,  spi_mapping[spi->port].pin_mosi->number,   spi_mapping[spi->port].gpio_af );
	GPIO_PinAFConfig( gpio_mapping[spi->chip_select].bank,    gpio_mapping[spi->chip_select].number,    GPIO_AF_SPI1 );
#endif
    ...
    spi_mapping[spi->port].peripheral_clock_func( spi_mapping[spi->port].peripheral_clock_reg,  ENABLE );
{
	...
	if ( g_spix_bulkmode!=1 ) // sekim XXXXXX
		SPI_I2S_ITConfig(SPI1, SPI_I2S_IT_RXNE, ENABLE);
...
#ifdef BUILD_WIZFI250 	// 2013.12.05 sskim
    if ( g_spix_bulkmode==1 ) // sekim XXXXXX
    {
		{
			wiced_spi_message_segment_t segment;
			
			init_spi_dma_buffer(&spi_rx_dma_buffer);											// 2014.01.07 sskim
			segment.tx_buffer = NULL;
			segment.length = DMA_BURST_SIZE;
			segment.rx_buffer = (void *)spi_rx_dma_buffer.buf[spi_rx_dma_buffer.push];			// 2014.01.07 sskim

			spi_dma_config(spi, &segment);
		}
		/* 2013.12.31 sskim dma initial */
		{
			/* Configure RX DMA interrupt on Cortex-M3 */
			NVIC_InitTypeDef  nvic_init_structure;
			nvic_init_structure.NVIC_IRQChannel                   = DMA2_Stream0_IRQn;
			nvic_init_structure.NVIC_IRQChannelPreemptionPriority = (uint8_t) 0x4;
			nvic_init_structure.NVIC_IRQChannelSubPriority        = 0x5;
			nvic_init_structure.NVIC_IRQChannelCmd                = ENABLE;
			NVIC_Init( &nvic_init_structure );

			/* Enable TC (transfer complete) and TE (transfer error) interrupts on source */
			DMA_ITConfig(spi_mapping[spi->port].rx_dma_stream, DMA_IT_TC | DMA_IT_TE, ENABLE);

			DMA_Cmd(spi_mapping[spi->port].rx_dma_stream, ENABLE);
		}
    }
#endif	// 2013.12.05 sskim
...
wiced_result_t wiced_spi_deinit( const wiced_spi_device_t* spi )
{
	...
	while(ring_buffer_used_space(&stdio_tx_buffer) != 0)
	...
	gpio_init_structure.GPIO_OType = GPIO_OType_PP;
    gpio_init_structure.GPIO_Speed = GPIO_Speed_2MHz;
    gpio_init_structure.GPIO_Pin   = ((uint32_t) (1 << spi_mapping[spi->port].pin_miso->number ));
    GPIO_Init( spi_mapping[spi->port].pin_clock->bank, &gpio_init_structure );
	wiced_gpio_output_low(WIZFI_SPI_MISO);
...
#ifdef BUILD_WIZFI250	// kaizen 20131126 ID1151 XON Signal
wiced_result_t wiced_check_spi_rx_status(void);
wiced_result_t wiced_check_spi_rx_status(void)
{
	uint32_t free_len;

	if(platfrom_spi_stdio_x == 2 && spi_rx_suspend == CTRL_REQ)
	{
		free_len = ring_buffer_free_space(&stdio_rx_buffer);

		if(free_len > SPI_RX_XON_POINT)
		{
			spi_rx_suspend = CTRL_NON;
			spi_tx_sig_cnt = SPI_GPIO_CLEAR_COUNT;
			wiced_gpio_output_high( WIZFI_SPI_REQ );
		}
	}
...
static wiced_result_t platform_uart_init( wiced_uart_t uart, const wiced_uart_config_t* config, wiced_ring_buffer_t* optional_rx_buffer )
{
...
#if 0	// kaizen 20130514 - For using USART Hardware Flow Control
    ...
    if ( uart_mapping[uart].pin_cts && (config->flow_control == FLOW_CONTROL_RTS || config->flow_control == FLOW_CONTROL_CTS_RTS) )
    {
...
#ifndef BUILD_WIZFI250 //MikeJ 130507 ID1063 - USART HW Flow Control problem
        gpio_init_structure.GPIO_Mode  = GPIO_Mode_AF;
        gpio_init_structure.GPIO_OType = GPIO_OType_OD;
        gpio_init_structure.GPIO_PuPd  = GPIO_PuPd_NOPULL;
        ...
        gpio_init_structure.GPIO_PuPd  = GPIO_PuPd_NOPULL;
        GPIO_Init( uart_mapping[uart].pin_rts->bank, &gpio_init_structure );
		GPIO_WriteBit(uart_mapping[uart].pin_rts->bank, (uint16_t) (1 << uart_mapping[uart].pin_rts->number), Bit_RESET);
#endif
#if 0	// kaizen 20130514 - For using USART Hardware Flow Control
    }
#endif
    ...
    /* Enable USART */
#ifndef BUILD_WIZFI250//MikeJ 130415 ID1005 - When booting, if there is a lot of USART input, Hang-up occur.
...
#ifdef BUILD_WIZFI250 //MikeJ 130528 ID1072 - Improve UART Performance
	uart_mapping[uart].usart->CR1 |= USART_CR1_OVER8;
#endif
...
wiced_result_t wiced_uart_receive_bytes( wiced_uart_t uart, void* data, uint32_t size, uint32_t timeout )
{
    ...
    {
#ifdef BUILD_WIZFI250 //MikeJ 130528 ID1072 - Improve UART Performance
		if(GPIO_ReadOutputDataBit(uart_mapping[uart].pin_rts->bank,
		(uint16_t) (1 << uart_mapping[uart].pin_rts->number))==Bit_SET
		&& ring_buffer_free_space(uart_interfaces[uart].rx_buffer)>30) {
			GPIO_WriteBit(uart_mapping[uart].pin_rts->bank,
			 (uint16_t) (1 << uart_mapping[uart].pin_rts->number), Bit_RESET);	//GPIO_WriteBit(GPIOC, GPIO_Pin_1, Bit_RESET);
		}
#endif
        while (size != 0)
        {
            ...
            // sekim 20131125 add platfrom_spi_stdio_x
#ifdef BUILD_WIZFI250
            if ( platfrom_spi_stdio_x!=platfrom_spi_stdio )
            {
           		void quick_change_sdtio_mode(uint8_t mode);
           		quick_change_sdtio_mode(platfrom_spi_stdio_x);
            }
#endif
...
void spi1_irq( void ) //MikeJ - SPI
{
#ifdef BUILD_WIZFI250
	if(SPI_I2S_GetITStatus(SPI1, SPI_I2S_IT_TXE) == SET)
	{
		if(spi_rx_previous != spi_rx_suspend) {
			if(spi_rx_suspend == CTRL_REQ) {
				spi_rx_previous = CTRL_REQ;
				spi_rx_suspend =  CTRL_RDY;
				SPI1->DR = SPI_ESC;
			} else if(spi_rx_suspend == CTRL_NON) {
				spi_rx_previous = CTRL_NON;
				spi_rx_suspend =  CTRL_RDY;
				SPI1->DR = SPI_ESC;
			} else if(spi_rx_suspend == CTRL_RDY) {
				spi_rx_suspend = spi_rx_previous;
				if(spi_rx_previous == CTRL_REQ)
					SPI1->DR = SPI_XOFF;
				else SPI1->DR = SPI_XON;
			}

			spi_tx_sig_cnt = SPI_GPIO_CLEAR_COUNT;
			if(spi_rx_stop == WICED_FALSE) wiced_gpio_output_high( WIZFI_SPI_REQ );
		}
		else if(ring_buffer_used_space(&stdio_tx_buffer) == 0 || spi_tx_suspend == CTRL_REQ)
		{
			SPI1->DR = SPI_NULL;
			// Deactivate Transmission Request through GPIO if it is Active
			if(spi_tx_sig_cnt > 0) {
				spi_tx_sig_cnt--;
				if(spi_tx_sig_cnt <= 0) wiced_gpio_output_low( WIZFI_SPI_REQ );
			}
		}
		else
		{
			...
			static uint8_t esc = 0xFF;

			if(esc == 0xFF) {
				ring_buffer_get_data(&stdio_tx_buffer, &pch, &tlen);
				ring_buffer_consume(&stdio_tx_buffer, 1);
				if(*pch == 0xF0) {
					SPI1->DR = SPI_ESC;
					esc = SPI_F0;
				} else if(*pch == 0xF1) {
...
//#if 0	// 2013.12.09 sskim
	if ( g_spix_bulkmode!=1 ) // sekim XXXXXX
	{
		if(SPI_I2S_GetITStatus(SPI1, SPI_I2S_IT_RXNE) == SET)
		{
			uint8_t tch;
			uint32_t free_len;
			static uint8_t esc = RESET;
			static uint8_t sync[2] = {SPI_ESC, SPI_SYNC};
			...
			if(tch == SPI_ESC) {
				...
				case SPI_SYNC: // Handle Sync Signal
					if(ring_buffer_free_space(&stdio_tx_buffer) >= 2)
						ring_buffer_write(&stdio_tx_buffer, sync, 2);
					//GPIO_ResetBits( GPIOB, (uint16_t) (1 << 6) );
					return;
				case SPI_XON: // Handle Xon Signal
					spi_tx_suspend = CTRL_NON;
					return;
				case SPI_XOFF: // Handle Xoff Signal
					spi_tx_suspend = CTRL_REQ;
					return;
				case SPI_ERR: // Handle Error Signal
					// ToDo
					return;
				case SPI_ESC: // Lost ESC Data
					esc = SET;	//Just Ignore previous ESC
					return;
				default:
					// ToDo
					return;
			...
			free_len = ring_buffer_free_space(&stdio_rx_buffer);
			if(free_len <= SPI_RX_XON_POINT) {
				if(spi_rx_suspend == CTRL_NON) {	// 'IF' is needed for check that it is not in sending action (CTRL_RDY)
					spi_rx_suspend = CTRL_REQ;
					spi_tx_sig_cnt = SPI_GPIO_CLEAR_COUNT;
					if(spi_rx_stop == WICED_FALSE) wiced_gpio_output_high( WIZFI_SPI_REQ );
				}
				if(free_len == 0) {
					GPIO_ResetBits( GPIOB, (uint16_t) (1 << 6) );
					goto SEM_CALC_END; //return;
				}
			}

			ring_buffer_write(&stdio_rx_buffer, &tch, 1);

			SEM_CALC_END:

			// sekim 20131125 add platfrom_spi_stdio_x
			platfrom_spi_stdio_x = 2;

			if((uart_interfaces[0].rx_size > 0) && (ring_buffer_used_space(&stdio_rx_buffer) >= uart_interfaces[0].rx_size))
			{
...
//#endif	// 2013.12.09 sskim
#endif
}

#if 1	// 2013.12.05 sskim
#ifdef BUILD_WIZFI250	// 2013.12.09 sskim
uint32_t spi_rx_data_cnt = 0;
uint32_t spi_rx_drop_cnt = 0;
uint32_t spi_stdio_drop_cnt = 0;
uint32_t spi_tx_buffer_cnt = 0;
uint32_t spi_rx_buffer_cnt = 0;
uint32_t spi_int = 0;
uint32_t spi_thread_cnt = 0;
uint32_t ndtr = 0;
...
void spi1_rx_dma_irq(void)
{
	...
	ndtr = spi_mapping[0].rx_dma_stream->NDTR;
	ndtr = DMA_BURST_SIZE - ndtr;
	spi_tx_buffer_cnt += ndtr;

	push_dma_data(&spi_rx_dma_buffer, ndtr);
	spi_mapping[0].rx_dma_stream->M0AR = (uint32_t)spi_rx_dma_buffer.buf[spi_rx_dma_buffer.push];

	// sekim XXXXX
	host_rtos_set_semaphore(&dma_mutex, WICED_TRUE);
	spi_rx_buffer_cnt += ndtr;

	sprintf(szXXXX3, "XXX 203 : (%lu,%lu,%lu,%lu,%lu), (%lu,%lu,%lu), (%lu,%lu)\r\n",
			spi_rx_dma_buffer.buf_len[0],spi_rx_dma_buffer.buf_len[1],spi_rx_dma_buffer.buf_len[2],spi_rx_dma_buffer.buf_len[3],spi_rx_dma_buffer.buf_len[4],
			spi_rx_dma_buffer.push, spi_rx_dma_buffer.pop, spi_rx_dma_buffer.len,
			spi_rx_dma_buffer.overflow, spi_rx_dma_buffer.underflow);
	strcat(szXXXX2, szXXXX3);

	DMA_Cmd(spi_mapping[0].rx_dma_stream, ENABLE);
#endif	// 2013.12.09 sskim
}
...
void spi1_tx_dma_irq(void)
{
	...
	{
		DMA2->HIFCR |= DMA_HISR_TEIF5;
	}
}
#endif	// 2013.12.05 sskim

#ifdef BUILD_WIZFI250	// 2013.12.18 sskim
static uint32_t polling_cnt = 0;
uint32_t poll_event_cnt = 0;
uint32_t pre_dma_data = 0;
void dma_polling_thread_hndlr( uint32_t arg )
{
	static uint32_t pre_dma_data_num = 0xffffffff;
	arg++;
	...
	while(1)
	{
		...
		if(pre_dma_data_num == spi_mapping[0].rx_dma_stream->NDTR)
		{
			if(pre_dma_data_num != DMA_BURST_SIZE)
				polling_cnt++;

			if(polling_cnt >= 10)
			{
//				spi_test_cnt = spi_tx_buffer_cnt;
				pre_dma_data = pre_dma_data_num;

				DMA_Cmd(spi_mapping[0].rx_dma_stream, DISABLE);	
				polling_cnt = 0;
				pre_dma_data_num = 0xffffffff;
				poll_event_cnt++;
			}
		}
		else
		{
			pre_dma_data_num = spi_mapping[0].rx_dma_stream->NDTR;
		}
	}
}
...
extern uint8_t g_wxModeState;
extern uint8_t WXNetwork_NetTx(uint8_t scid, void *buf, uint32_t len);
void spi_dma_thread_hndlr( uint32_t arg )
{
	uint32_t	i, l_ndtr = 0;
	...
	while(1)
	{
		...
		host_rtos_get_semaphore(&dma_mutex, WICED_NEVER_TIMEOUT, WICED_TRUE );
		//host_rtos_get_semaphore(&dma_mutex, 10, WICED_TRUE );

		spi_thread_cnt++;

		free_len = ring_buffer_free_space(&stdio_rx_buffer);

		sprintf(szXXXX3, "XXX 205 : (%lu,%lu,%lu,%lu,%lu), (%lu,%lu,%lu), (%lu,%lu), (%lu)\r\n",
				spi_rx_dma_buffer.buf_len[0],spi_rx_dma_buffer.buf_len[1],spi_rx_dma_buffer.buf_len[2],spi_rx_dma_buffer.buf_len[3],spi_rx_dma_buffer.buf_len[4],
				spi_rx_dma_buffer.push, spi_rx_dma_buffer.pop, spi_rx_dma_buffer.len,
				spi_rx_dma_buffer.overflow, spi_rx_dma_buffer.underflow,
				free_len);
		...
		*/
		...
		DMA_ITConfig(spi_mapping[0].rx_dma_stream, DMA_IT_TC | DMA_IT_TE, DISABLE);
		ret = pop_dma_data(&spi_rx_dma_buffer, buf, &l_ndtr);
		DMA_ITConfig(spi_mapping[0].rx_dma_stream, DMA_IT_TC | DMA_IT_TE, ENABLE);

		sprintf(szXXXX3, "XXX 210 : (%lu,%lu,%lu,%lu,%lu), (%lu,%lu,%lu), (%lu,%lu), (%lu)\r\n",
				spi_rx_dma_buffer.buf_len[0],spi_rx_dma_buffer.buf_len[1],spi_rx_dma_buffer.buf_len[2],spi_rx_dma_buffer.buf_len[3],spi_rx_dma_buffer.buf_len[4],
				spi_rx_dma_buffer.push, spi_rx_dma_buffer.pop, spi_rx_dma_buffer.len,
				spi_rx_dma_buffer.overflow, spi_rx_dma_buffer.underflow,
				free_len);
		...
		spi_rx_buffer_cnt += l_ndtr;

		if(g_wxModeState == WX_MODE_DATA)
		{
			WXNetwork_NetTx((uint8_t)0, (void *)buf, l_ndtr);
		}
		else
		{
			for(i = 0 ; i < l_ndtr ; i++)
...
#ifdef BUILD_WIZFI250
    // sekim 20131125 add platfrom_spi_stdio_x
    platfrom_spi_stdio_x = 1;
#endif

    // Notify thread if sufficient data are available
	usedlen = ring_buffer_used_space( uart_interfaces[0].rx_buffer );
    if ( ( uart_interfaces[0].rx_size > 0 ) && ( usedlen >= uart_interfaces[0].rx_size ) )
    {
        host_rtos_set_semaphore( &uart_interfaces[0].rx_complete, WICED_TRUE );
        uart_interfaces[0].rx_size = 0;
    }

	if(usedlen > uart_interfaces[0].rx_buffer->size - 30)
	{
		GPIO_WriteBit(uart_mapping[STDIO_UART].pin_rts->bank,
		(uint16_t) (1 << uart_mapping[WICED_UART_1].pin_rts->number), Bit_SET);
	}
#endif
}
...
#else
    // sekim 20131125 Add SPI Interface ==>
	if ( platfrom_spi_stdio==0 || platfrom_spi_stdio==1 )
	{
		wiced_uart_transmit_bytes( STDIO_UART, (void*)str, len );
	}
	if ( platfrom_spi_stdio==0 || platfrom_spi_stdio==2 )
	{
		uint32_t freelen;
		freelen = ring_buffer_free_space(&stdio_tx_buffer);
		...
		ring_buffer_write(&stdio_tx_buffer, (uint8_t*)str, len);

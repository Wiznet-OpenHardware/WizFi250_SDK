...
// sekim 20130109 add last_joined_ssid
#if 0 //MikeJ 130806 ID1116 - Couldn't display 32 characters SSID
char last_joined_ssid[32] = "";
char last_started_ssid[32] = "";
#else
char last_joined_ssid[32+1] = "";
char last_started_ssid[32+1] = "";
#endif
...
wiced_result_t wiced_init( void )
{
    ...
    // kaizen 20130408 device_configured Flag와는 상관 없이, DCT에 있는 country_code를 적용하도록 수정
...
wiced_result_t wiced_join_ap( void )
{
    ...
    int retries;
    const wiced_config_ap_entry_t* ap;
//#if 1 //MikeJ 130806 ID1116 - Couldn't display 32 characters SSID
#ifdef BUILD_WIZFI250
	char tmp_ssid[32+1];
#endif
	...
	for ( retries = WICED_JOIN_RETRY_ATTEMPTS; retries > 0; --retries )
    {
// sekim 20140504 ID1174 Stopped Joining by <Esc> (by ShinHeung)
#ifdef BUILD_WIZFI250
		{
			extern void W_DBG(const char *format, ...);
			uint8_t ch_esc = 0;
			if ( wiced_uart_receive_bytes(WICED_UART_1, &ch_esc, 1, 1)==WICED_SUCCESS )
			{
				if ( ch_esc==0x1B )
				{
					W_DBG("Stopped Joining by <ESC>");
					break;
				}
			}
		}
        ...
        for ( a = 0; a < CONFIG_AP_LIST_SIZE; ++a )
        {
            ...
            if ( ap->details.SSID.len != 0 )
            {
...
#ifdef BUILD_WIZFI250
#if 0 //MikeJ 130806 ID1116 - Couldn't display 32 characters SSID
            	// sekim 20130321 Major-WiFi-MSG ==> W_EVT
            	extern void W_EVT(const char *format, ...);
            	W_EVT("Joining : %s\r\n", (char*)ap->details.SSID.val);
#else
            	// sekim 20130321 Major-WiFi-MSG ==> W_EVT
            	extern void W_EVT(const char *format, ...);

				memcpy(tmp_ssid, ap->details.SSID.val, ap->details.SSID.len);
				tmp_ssid[ap->details.SSID.len] = 0;

				// sekim 20150416 add AT+MEVTFORM, AT+MEVTDELAY for Coway
            	//W_EVT("Joining : %s\r\n", tmp_ssid);
				if ( g_wxProfile.event_msg_format[9]=='1' )	W_EVT("Joining : %s\r\n", tmp_ssid);
#endif
#else
                ...
                // sekim 20140731 ID1183 WEP Shared Problem by ZionTek
        		if ( ap->details.security==WICED_SECURITY_WEP_PSK || ap->details.security==WICED_SECURITY_WEP_SHARED )
        		{
        			retries--;
        		}
                ...
                {
					///////////////////////////////////////////////////////////////////////////////////////////////
					// sekim 20130806 ID1116 & 20130321 Major-WiFi-MSG ==> W_EVT
					memcpy(last_joined_ssid, (char*)ap->details.SSID.val, ap->details.SSID.len );
					last_joined_ssid[ap->details.SSID.len] = 0;
#ifdef BUILD_WIZFI250
					// sekim 20150416 add AT+MEVTFORM, AT+MEVTDELAY for Coway
	            	//W_EVT("Successfully joined : %s\r\n", tmp_ssid);
					if ( g_wxProfile.event_msg_format[9]=='1' )	W_EVT("Successfully joined : %s\r\n", tmp_ssid);
#else
...
#if 0	// kaizen 20130703 ID1080 - Do not use PMK Cache until release SDK of stable version
                ...
                else
                {
                	...
                	// sekim 20130321 Major-WiFi-MSG ==> W_EVT
#if 0 //MikeJ 130806 ID1116 - Couldn't display 32 characters SSID
                    W_EVT("Failed to join: %s\r\n", (char*)ap->details.SSID.val);
#else
					// sekim 20150416 add AT+MEVTFORM, AT+MEVTDELAY for Coway
	            	//W_EVT("Failed to join: %s\r\n", tmp_ssid);
					if ( g_wxProfile.event_msg_format[9]=='1' )	W_EVT("Failed to join: %s\r\n", tmp_ssid);
#endif
#else
...
wiced_result_t wiced_leave_ap( void )
{
    ...
    // sekim 20130109 add last_joined_ssid
    memset(last_joined_ssid, 0, sizeof(last_joined_ssid));
...
wiced_result_t wiced_wifi_scan_networks( wiced_scan_result_handler_t results_handler, void* user_data )
{
    ...
    const wiced_scan_extended_params_t extparam = { 5, 70, 150, 1 };		// kaizen ID1170 Modified Scan Timing

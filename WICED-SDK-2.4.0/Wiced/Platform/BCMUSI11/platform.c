...
wiced_result_t wiced_platform_init( void )
{
    ...
    {
    	// sekim 20130403 Notice Watchdog Reset
    	...
    	void WXS2w_StatusNotify(unsigned char status, unsigned int arg);
		#define WXCODE_RESET              10 // from wx_defines.h
    	// sekim 20130801 Watchdog Reset => Event
    	W_DBG("Watchdog reset occurred previously.");
    	WXS2w_StatusNotify(WXCODE_RESET, 0);

    	// kaizen 20130411 ID1038	When occurring watchdog reset, Execute NVIC_SystemReset() to clean the system variable.
    	wiced_result_t wiced_rtos_delay_milliseconds( uint32_t milliseconds );
    	wiced_rtos_delay_milliseconds(10);
    	NVIC_SystemReset();
    }
#endif
#if 0	// kaizen 20130425 delete to change platform BCM9WCDUSI09 to BCMUSI11
    if ( WICED_TRUE == watchdog_check_last_reset() )
    {
        WPRINT_PLATFORM_ERROR(("WARNING: Watchdog reset occured previously. Please see watchdog.c for debugging instructions.\r\n"));
    }
...
void init_platform( void )
{
    ...
    // sekim 20131125 Add SPI Interface, kaizen 20140430 ID1173 Moved initial routine of WIZFI_SPI_REQ
...
#if 1	// kaizen 20131119 ID1140 Function Button Pull-Up
	wiced_gpio_init(WICED_GPIO_3, INPUT_PULL_UP );
#endif

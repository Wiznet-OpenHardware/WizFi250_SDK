...
static START_OF_HTTP_PAGE_DATABASE(web_pages)
    ...
    { "/apps/temp_control/main.html",    "text/html",                WICED_STATIC_URL_CONTENT,  .url_content.static_data  = {resource_apps_DIR_temp_control_DIR_main_html, sizeof(resource_apps_DIR_temp_control_DIR_main_html)-1}, },
...
void application_start( )
{
    ...
    /* Initialise Xively data. Xively data are shared among multiple threads; therefore a mutex is required */
...
static int process_temperature_update( const char* url, wiced_tcp_stream_t* stream, void* arg )
{
    ...
    wiced_tcp_stream_write(stream, resource_apps_DIR_temp_control_DIR_data_html_set_start,  sizeof(resource_apps_DIR_temp_control_DIR_data_html_set_start)-1);
    temp_char_buffer_length = sprintf(temp_char_buffer, "%.1f", setpoint_celcius);
    wiced_tcp_stream_write(stream, temp_char_buffer, temp_char_buffer_length );
    wiced_tcp_stream_write(stream, resource_apps_DIR_temp_control_DIR_data_html_set_mid,    sizeof(resource_apps_DIR_temp_control_DIR_data_html_set_mid)-1);
    temp_char_buffer_length = sprintf(temp_char_buffer, "%.1f", setpoint_fahrenheit);
    wiced_tcp_stream_write(stream, temp_char_buffer, temp_char_buffer_length );
    wiced_tcp_stream_write(stream, resource_apps_DIR_temp_control_DIR_data_html_set_end,    sizeof(resource_apps_DIR_temp_control_DIR_data_html_set_end)-1);

    wiced_tcp_stream_write(stream, resource_apps_DIR_temp_control_DIR_data_html_page_end,   sizeof(resource_apps_DIR_temp_control_DIR_data_html_page_end)-1);

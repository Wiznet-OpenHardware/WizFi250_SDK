...
// sekim 20150512 DHCP PATCH Test => 20150519 Recover
//#include "nx_ip.h"
//#include "nx_udp.h"
...
void host_network_process_ethernet_data( wiced_buffer_t buffer, wiced_interface_t interface )
{
...
#endif
    {
        ...
        if ( ( ethertype == WICED_ETHERTYPE_IPv4 ) || ethertype == WICED_ETHERTYPE_IPv6 )
        {
        	NX_IPV4_HEADER* ip_header  = (NX_IPV4_HEADER *)packet_ptr->nx_packet_prepend_ptr;
            NX_UDP_HEADER* udp_header = (NX_UDP_HEADER*)(packet_ptr -> nx_packet_prepend_ptr + sizeof(NX_IPV4_HEADER));
            ...
            if ( ( sender_port == 67 ) && ( destination_ip != 0xFFFFFFFF ) && ( destination_ip != ip_ptr[(int)interface]->nx_ip_interface[0].nx_interface_ip_address ) )
            {

...
static struct sockaddr_in       server_local_addr;

static void web_server_proc( void * thread_input )
{
    run_webserver( (void*) &server_local_addr, server_url_list );
...
{
    memcpy( &server_local_addr, bind_address_in, sizeof( struct sockaddr_in ) );
...
{
    int socket;
    struct sockaddr_in * bind_address = (struct sockaddr_in*) bind_address_in;
    ...
    while ( web_server_quit_flag == 0 )
    {
        ...
        if ( bytes_received > 0 )
        {
            ...
            if ( 1 == process_url_request( server_url_list, start_of_url, (int) end_of_url - (int) start_of_url, (void*) accepted_socket_hnd, htonl(other_end_addr) ) )
            {
                web_server_quit_flag = 1;
            }

...
#else
#if !defined(TARGETOS_nucleus) && !defined(MACOSX) && !defined(vxworks) && !defined(TARGET_wiced)
#include <malloc.h>
#endif
#endif /* SYMBIAN */
#include <assert.h>
...
rwl_var_getbuf(void* wl, const char* iovar, void* param, int param_len, void** buf_ptr)
{
	...
	*buf_ptr = rwl_buf;
	...
	return wl_ioctl(wl, WLC_GET_VAR, &rwl_buf[0], WLC_IOCTL_MAXLEN, FALSE);
}
...
rwl_var_setbuf(void* wl, const char* iovar, void* param, int param_len)
{
	...
	return wl_ioctl(wl, WLC_SET_VAR, &rwl_buf[0], len, TRUE);
}
...
remote_server_exec(int argc, char **argv, void *wl)
{
	...
	while (1) {
		...
		if ( ( g_rem_ptr->msg.len > WLC_IOCTL_MAXLEN ) ||
		        ( ( buf_ptr = malloc(g_rem_ptr->msg.len)) == NULL ) ) {
...
#endif /* defined(LINUX) || defined(SYMBIAN) || defined(TARGETOS_nucleus) || defined(TARGET_wiced) */
			g_rem_ptr->msg.flags = REMOTE_GET_IOCTL;
		} /* REMOTE_GET_IOCTL */

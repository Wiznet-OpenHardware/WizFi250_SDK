...
void start_dns_server( struct sockaddr_in * local_addr );
...
void start_dns_server( struct sockaddr_in * local_addr )
{
    xTaskCreate( dns_thread, (signed char*)"DNS thread", DNS_STACK_SIZE/sizeof( portSTACK_TYPE ), local_addr, DEFAULT_THREAD_PRIO, &dns_thread_handle);
}
...
static void dns_thread( void * thread_input )
{
    struct sockaddr_in*   input_addr = (struct sockaddr_in*) thread_input;
    ...
    memcpy( my_addr, input_addr, sizeof( struct sockaddr_in ) );
